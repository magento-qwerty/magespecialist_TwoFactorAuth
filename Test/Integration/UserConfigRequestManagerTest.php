...
class UserConfigRequestManagerTest extends TestCase
{
    ...
    {
        $this->assertTrue($this->manager->isConfigurationRequiredFor((string)$this->user->getId()));
    }
    ...
    {
        $this->tfa->getProvider(Google::CODE)->activate($this->user->getId());
        $this->assertFalse($this->manager->isConfigurationRequiredFor((string)$this->user->getId()));
    }
    ...
    {
        $this->aclBuilder->getAcl()->deny(null, 'MSP_TwoFactorAuth::config');
    ...
    public function testSendAppConfigRequest(): void
    {
        ...
        $this->assertTrue($this->tokenManager->isValidFor((string)$this->user->getId(), $token));
    }
